...
d <- read.csv("biotime_results/biotime_common.csv") %>%
  as_tibble() %>% 
...
lcvp %>%
  ...
  slice(1) %>%
  ungroup() %>%
  write_csv("biotime_results/bogota_lcvp.csv")
...
write_csv(fishbase, "biotime_results/bogota_fishbase.csv")
...
birds <- tibble(parsed = d$parsed,
  ...
  select(-code)
write_csv(birds, "biotime_results/bogota_ebird.csv")
