...
if (!file.exists("data_cleaned/all_pkgs_df.Rds")) {
...
all_pkgs_df <- readRDS("data_cleaned/all_pkgs_df.Rds")

# Package igraph network -------------------------------------------------------

# Edge list data.frame
if (!file.exists("data_cleaned/taxo_pkgs_igraph.Rds")) {
  dep_df = all_pkgs_df %>%
    select(pkg = ref, deps) %>%
    tidyr::unnest(deps) %>%
  ...
  saveRDS(taxo_df, "data_cleaned/taxo_pkgs_df.Rds", compress = TRUE)
}
taxo_df <- readRDS("data_cleaned/taxo_pkgs_df.Rds")  
...
if (!file.exists("data_cleaned/db_df.Rdata")) {
  ...
  save(db_links, all_db, file = "data_cleaned/db_df.Rdata")
}
load("data_cleaned/db_df.Rdata")
